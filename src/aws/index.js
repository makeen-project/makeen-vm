...
export default class EC2Client {
  ...
  async listInstances() {
    const listInstances = Promise.promisify(this.ec2.describeInstances, { context: this.ec2 });
  ...
  async turnInstancesOff(instanceIds) {
    const stopInstances = Promise.promisify(this.ec2.stopInstances, { context: this.ec2 });
  ...
  async turnInstancesOn(instanceIds) {
    const startInstances = Promise.promisify(this.ec2.startInstances, { context: this.ec2 });
