...
export default class AzureClient {
  ...
  async init(azureCredentials) {
    ...
    const { /* appId, key, */ tenantId, subscriptionId, user, password } = azureCredentials;

    const login = Promise.promisify(msRestAzure.loginWithUsernamePassword, msRestAzure);
    ...
    this.storageClient = new StorageManagementClient(credentials, subscriptionId);
  ...
  async listInstances() {
    ...
    const getInstancesTask = groups
      .map(async ({ name }) => new Promise((resolve, reject) => {
        this
          .computeClient
          .virtualMachines
          .list(name, (err, result) => {
            if (err) {
    ...
    const getInstancesWithStateTasks = instancesByGroup
    .map((group) => {
      const groupName = Object.keys(group)[0];
      const groupInstances = group[groupName];

      return groupInstances
        .map(({ name }) => new Promise((res, rej) => {
          this
          .computeClient
          .virtualMachines
          .get(groupName, name, { expand: 'instanceView' }, (err, result) => {
            if (err) {
              return rej(err);
            }
            return res(result);
  ...
  async turnInstancesOn(ids) {
    const stopTasks = ids.map((id) => {
      ...
      return new Promise((resolve, reject) => {
        this.computeClient.virtualMachines.start(resourceGroupName, vmName, (err, _result) => {
          if (err) {
            return reject(err);
          }
          return resolve(_result);
