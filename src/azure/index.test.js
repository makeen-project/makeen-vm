...
test('should turn instance off', async () => {
  ...
  const activeMachine = instances
    .find(({ instanceView: { statuses } }) => statuses.find(({ code }) => code === 'PowerState/running'));
  if (!activeMachine) {
...
test('should turn instance on', async () => {
  ...
  const activeMachine = instances
    .find(({ instanceView: { statuses } }) => statuses.find(({ code }) => code === 'PowerState/stopped'));
  if (!activeMachine) {
    expect(true).toBe(false);
    return;
  }
